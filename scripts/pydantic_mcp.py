...
class FactsAboutDuy(BaseModel):
    ...
    rationale: Annotated[
        str,
        Field(
            description="Explain briefly your thinking in answering the questions above."
        ),
    ]
...
async def main() -> None:
    ...
    agent = Agent(
        ...
        system_prompt="Answer questions as provided.",
        toolsets=[server],
    )

    async with agent:
        result = await agent.run(
            "Answer questions as provided in the output structure."
        )
